...
if [ -f data_writes.gp ]; then
	rm data_writes.gp
fi
for i in $(seq 0 $end); do
	#line[i]="${file1col1[i]} ${file1col2[i]} ${file2col2[i]}"
	echo "${file1col1[i]} ${file1col2[i]} ${file2col2[i]} ${file3col2[i]}" >> data_writes.gp
...

{
	...
	echo "set output 'writes.png'"
	echo "set xrange [1:31]"
	echo "set xlabel \"Clients\""
	echo "set ylabel \"Latency [ms]\""
	echo "plot 'data_writes.gp' using 1:2 title \"No Pgpool\" pt 7 ps 1, '' using 1:3 title \"2 slaves\" pt 7 ps 1, '' using 1:4 title \"N slaves\" pt 7 ps 1"
} > writes.gp
